...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring, Writer& output )
{
  ...
  if ( first_unassembled_ == first_unacceptable_ )
    return; // no bytes can be pushed to the stream
  ...
  if ( eof_flag_ && first_index >= eof_idx_ )
    return;

  if ( first_index >= first_unacceptable_ )
    return; // beyond capacity

  if ( first_index + data.size() <= first_unassembled_ )
    return; // already in Bytestream
